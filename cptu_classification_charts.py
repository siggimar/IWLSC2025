...
'''
    This script defines & presents 2D calssification models
'''
...
class general_model(): 
    ...
    def parse_raw_def( self, model_def ):
        ...
        self.sensitive_region = model_def['desc']['sensitive']
    ...
    class region_2d():
        def __init__( self, parent, index, name, id_loc, x, y ):
            ...
            # report back
            self.parent.x_lim[0] = min(self.parent.x_lim[0], self.min_x)
            self.parent.x_lim[1] = max(self.parent.x_lim[1], self.max_x)
            self.parent.y_lim[0] = min(self.parent.y_lim[0], self.min_y)
            self.parent.y_lim[1] = max(self.parent.y_lim[1], self.max_y)
...
if __name__=='__main__': # running this script displays each chart
    if False:

        for m in model_defs:
            cl=general_model(model_defs[m], fill_regions=False)
            cl.plot( letter='' )

    else:
        from copy import deepcopy
        some_model = deepcopy(model_defs['robertson_90_fr'])

        for region in some_model['regions']:
            x, y = some_model['regions'][region]['xy']
            x_new = np.array(x) * np.array(y)
            some_model['desc']['x_bounds'][1] = max(some_model['desc']['x_bounds'][1],max(x_new))
            some_model['regions'][region]['xy'][0] = x_new.tolist()


        cl=general_model(some_model, fill_regions=False)
        cl.prep_figure( '' )
        if True:
            cl.ax.set_yscale('linear')
            cl.ax.set_xscale('linear')

        cl.ax.set_xlabel( r"$\frac{ f_s }{ \sigma' _v }$" +' (-)', fontsize=16 )
        plt.show()